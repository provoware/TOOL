...
def start_gui(db_path, theme, themes, theme_dir, backup_dir):
    """Start the PySimpleGUI interface."""
    ...
    sg.theme(theme if isinstance(theme, str) else 'DefaultNoMoreNag')
    layout = [
        [sg.Text('Playlist', font=('DejaVu Sans', 16, 'bold'))],
        [sg.Listbox(values=[], size=(40, 10), key='-LIST-', enable_events=True)],
        [sg.Button('Abspielen', key='-PLAY-'), sg.Button('Pause', key='-PAUSE-'), sg.Button('Stopp', key='-STOP-')],
        [sg.Button('Hinzufügen'), sg.Button('Entfernen')],
        [
            sg.Text('Lautstärke'),
            sg.Slider((0, 1), default_value=0.8, resolution=0.05, orientation='h', key='-VOL-', enable_events=True),
        ],
    ...
    while True:
        ...
        if ev == 'Hinzufügen':
            files = sg.popup_get_file(
                'Datei auswählen', multiple_files=True, file_types=(('Audio', '*.mp3;*.wav;*.ogg'),)
            )
            if files:
